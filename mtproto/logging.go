...
func (h SimpleLogHandler) StringifyMessage(isIncoming bool, dc int32, msg TL, id int64) string {
	var text string
	switch x := msg.(type) {
	...
	return fmt.Sprintf("[DC%d] ", dc) + text
}
...
func (h SimpleLogHandler) Message(isIncoming bool, dc int32, msg TL, id int64) {
	h.Log(DEBUG, nil, h.StringifyMessage(isIncoming, dc, msg, id))
}
...
func (h ColorLogHandler) Message(isIncoming bool, dc int32, msg TL, id int64) {
	h.Log(DEBUG, nil, h.StringifyMessage(isIncoming, dc, msg, id))
}
...
func (l Logger) Message(isIncoming bool, dc int32, message TL, id int64) {
	l.Hnd.Message(isIncoming, dc, message, id)
}
